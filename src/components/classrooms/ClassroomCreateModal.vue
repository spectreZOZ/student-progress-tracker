...
const handleSubmit = async () => {
  ...
  try {
    const formData = {
      ...
      teacher: form.teacherId || selectedTeacher.value, // Send teacherId to backend
    };

    await classroomStore.addClassroom(formData);
    emit("created");
    resetForm();
  } catch (err: any) {
    error.value = err.message || "Failed to create classroom";
  } finally {
    loading.value = false;
  }
};
